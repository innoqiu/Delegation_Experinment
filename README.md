...
- Recap底部将真人讨论准备拆为对方预期、任务内第一印象和后续联系笔记三项，不在真人讨论前引导参与者修改配置。真人讨论完成后，实验员可让参与者在Agent配置页右上角输入`PROFILE_REVISION_PASSWORD`，解锁最近一条已完成任务的原配置／修改副本对照；修改副本与字段级diff保存在session中，不覆盖参与者原始Profile。
...
- 管理员可从历史页一键下载整理后数据ZIP：一份合并Markdown，以及“参与者三个Profile（含前后修改记录）”“参与者Recap与有效标记”“Agent交流记录与有效标记”三份结构化JSON。导出仅纳入有交流内容的已完成会话，排除已取消标记、失败／空会话、未使用账号、登录／同意元数据、模型配置、系统提示词、内部审核日志、原始store与API Key。
...
API smoke test会启动隔离的数据目录与本地模拟模型，验证首次登录知情同意、`admin_arklab`直接登录、固定recap schema、A/B结构一致性、登录权限、配置保存、双代理交互、三类标记、标记取消、三段讨论准备、会话级再配置diff、含Profile前后修改的整理后ZIP导出和历史记录。浏览器smoke test验证一般页面；review-flow browser test验证标记分组、取消、讨论准备、Recap中不存在提前再配置入口、Agent配置页右上角的密码门控、参与者原配置／修改副本对照，以及管理员对前后修改记录的只读查看；两者均需要本机安装Chrome。
