...
一个可独立部署的代理沟通实验系统。参与者通过编号登录并配置自己的代理、查看recap和相关对话；管理员配置OpenAI兼容模型、运行Task、查看全部历史记录与逐条评论。
...
- 参与者仅可访问自己的Agent配置、相关Task 1–3 session的完整transcript和自己的recap；Task 4只展示双方共享recap，不展示transcript。
- 管理员可查看所有已登录参与者、配置两个模型端点、获取模型列表并运行任意两位不同参与者的代理；Agent配置页同时按会话展示参与者真人讨论后的原配置／修改副本与字段diff。
- Task 1（社交计划）、Task 2（新关系介绍）与Task 3（固定10个共享支持额度的资源分配协商）均内置双代理交互提示词和recap结构。
- Task 4提供集中式基线：单个中立AI同时读取两位参与者的Profile 1–3，直接进行约束对齐并生成一份覆盖三个任务的双方共享Recap；它不代表任何一方、不模拟协商过程、不生成transcript，并只使用Agent 1模型端点。
...
- Recap底部将真人讨论准备拆为对方预期、任务内第一印象和后续联系笔记三项，不在真人讨论前引导参与者修改配置。真人讨论完成后，实验员可让参与者在Agent配置页右上角输入`PROFILE_REVISION_PASSWORD`，解锁三个Profile的原配置／修改副本对照；修改副本与字段级diff保存在session中，不覆盖参与者原始Profile。
- 管理员Recap页对Task 1–3按participant A／B左右对照展示双方独立recap；Task 4向管理员和双方参与者展示同一份覆盖三个任务的共享报告。旧Markdown记录会自动压缩为固定标题的报告视图。
- 管理员可在历史详情中永久删除已结束的记录；系统会二次确认，并禁止删除仍在运行或生成recap的记录。
- 管理员可从历史页一键下载整理后数据ZIP：一份合并Markdown，以及“参与者三个Profile（含前后修改记录）”“参与者Recap与有效标记”“Agent交流记录与有效标记”三份结构化JSON。导出纳入有交流内容的Task 1–3完成会话及具有共享Recap的Task 4完成会话；Task 4不会被伪装成Agent交流记录。导出排除已取消标记、失败／空会话、未使用账号、登录／同意元数据、模型配置、系统提示词、内部审核日志、原始store与API Key。
...
API smoke test会启动隔离的数据目录与本地模拟模型，验证首次登录知情同意、`admin_arklab`直接登录、固定recap schema、A/B结构一致性、登录权限、配置保存、双代理交互、Task 4单AI直接对齐与共享Recap、三类标记、标记取消、三段讨论准备、会话级再配置diff、含Profile前后修改的整理后ZIP导出和历史记录。浏览器smoke test验证一般页面；review-flow browser test验证标记分组、取消、讨论准备、Recap中不存在提前再配置入口、Agent配置页右上角的密码门控、参与者原配置／修改副本对照，以及管理员对前后修改记录的只读查看；两者均需要本机安装Chrome。
